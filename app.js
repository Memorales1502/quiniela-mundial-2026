...
const getKickoffDate = (match) => new Date(`${match.date}T${match.time || "00:00"}:00-06:00`);
...
const isInvalidPrediction = (prediction) => {
  return !prediction || prediction.invalid === true || prediction.homeScore === null || prediction.awayScore === null || prediction.homeScore === undefined || prediction.awayScore === undefined;
};

const scorePoints = (prediction, result) => {
  if (isInvalidPrediction(prediction)) return { points: 0, exact: false, winner: false, goals: 0 };
...
const hasResult = (matchId) => results[matchId]?.homeScore !== undefined && results[matchId]?.awayScore !== undefined;
...
async function savePrediction(match) {
  ...
  if (isLockedForInput(match, existing)) return alert("Este pronóstico ya está bloqueado y no puede modificarse.");
...
async function saveResult(match) {
  ...
  if (h === "" || a === "") return alert("Coloca resultado final de 90 minutos.");

  if (!confirm(`Guardar resultado oficial 90 minutos: ${match.home} ${h}-${a} ${match.away}?`)) return;
...
function card(match, prediction) {
  ...
  if (prediction?.submitted) lockText = '<span class="locked">Bloqueado por envío</span>';
  else if (hasResult(match.id)) lockText = '<span class="locked">Bloqueado por resultado</span>';
  else if (isStarted(match)) lockText = '<span class="locked">Bloqueado por inicio del partido</span>';
  ...
  return `<article class="match-card">
    ...
    <div>${result ? `Resultado 90': <b>${result.homeScore}-${result.awayScore}</b>` : "Resultado pendiente"}<br>${lockText}</div>
...
async function renderQuiniela() {
  ...
  const html = [];
  for (const match of MATCHES) html.push(card(match, await getPrediction(match.id)));
  $("matchesList").innerHTML = html.join("");
  MATCHES.forEach((match) => document.querySelector(`[data-save="${match.id}"]`)?.addEventListener("click", () => savePrediction(match)));
}
...
function renderResults() {
  $("resultsList").innerHTML = MATCHES.map((match) => {
    const result = results[match.id];
    return `<article class="match-card">
      <div><div class="teams">${match.home} vs ${match.away}</div><div class="meta">${match.date} ${match.time || ""} · Grupo ${match.group}</div></div>
...
function renderAdmin() {
  ...
  $("adminList").innerHTML = MATCHES.map((match) => `
    <article class="match-card">
      <div><div class="teams">${match.home} vs ${match.away}</div><div class="meta">${match.date} ${match.time || ""} · ${match.venue}</div></div>
      <div class="score-inputs">
...
async function buildScores(dateFilter = null) {
  ...
  predSnap.forEach((docSnap) => {
    ...
    if (!match || (dateFilter && match.date !== dateFilter)) return;
...
async function renderRanking() {
  ...
  $("rankingTable").innerHTML = `<table>
    <thead><tr><th>#</th><th>Jugador</th><th>Puntos</th><th>Exactos</th><th>Ganadores</th><th>Goles</th></tr></thead>
    <tbody>${rows.map((r, i) => `<tr><td>${i + 1}</td><td>${r.email}</td><td>${r.pts}</td><td>${r.exactos}</td><td>${r.ganadores}</td><td>${r.goles}</td></tr>`).join("")}</tbody>
  </table>`;
}
...
async function renderDaily() {
  ...
  $("dailyTable").innerHTML = `<table>
    <thead><tr><th>#</th><th>Jugador</th><th>Puntos del día</th><th>Exactos</th><th>Ganadores</th><th>Goles</th></tr></thead>
    <tbody>${rows.map((r, i) => `<tr><td>${i + 1}</td><td>${r.email}</td><td>${r.pts}</td><td>${r.exactos}</td><td>${r.ganadores}</td><td>${r.goles}</td></tr>`).join("")}</tbody>
  </table>`;
}
